...
const AdministrarSuscripciones = () => {
  ...
  const cancelarSuscripcion = async () => {
    ...
    try {
      ...
      setAlerta(error?.response?.data?.message || error.message || 'Error al cancelar la suscripción.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box display="flex">
      ...
      <Box flexGrow={1} p={3}>
        ...
        <Paper
          elevation={0}
          sx={{ p: 3, mb: 3, background: ui.heroBg, color: '#fff', borderRadius: 3, boxShadow: '0 12px 36px rgba(43, 43, 43, 0.3)' }}
        >
          <Stack direction={{ xs: 'column', md: 'row' }} alignItems="center" spacing={2} justifyContent="space-between">
            ...
            <Paper elevation={0} sx={{ p: 2, borderRadius: 2, bgcolor: ui.heroCardBg, border: `1px solid ${ui.borderSoft}`, minWidth: 280, backdropFilter: 'blur(4px)' }}>
              ...
              {planActual ? (
                <>
                  <Typography variant="body2" sx={{ mt: 1 }}>Plan: <b>{planActual?.nombre}</b></Typography>
                  <Typography variant="body2" sx={{ opacity: 0.85 }}>${toMoney(planActual?.costo)}/mes</Typography>
                </>
              ) : (
          ...
          {!suscripcionActiva ? (
            <>
              <Stack direction={{ xs: 'column', md: 'row' }} alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
                ...
                <Button onClick={() => empresaId && cargarDatos(empresaId)} disabled={loading}
                  startIcon={loading ? <CircularProgress size={16} /> : <AutorenewIcon />} variant="outlined"
                  sx={{ color: '#fff', borderColor: ui.borderSoft, '&:hover': { bgcolor: ui.outlineHover, borderColor: '#fff' } }}>
                      ...
                      <Card elevation={0} sx={{ height: '100%', bgcolor: '#111827', border: `1px solid ${featured ? ui.featuredRing : ui.borderSoft}`, boxShadow: featured ? ui.featuredShadow : 'none', borderRadius: 3 }}>
                        <CardContent>
                          ...
                          <Typography variant="h3" fontWeight={900} sx={{ color: ui.price, mt: 1, lineHeight: 1 }}>
                            ${toMoney(costo)}<Typography component="span" variant="subtitle2" sx={{ ml: 0.5, opacity: 0.8 }}>/mes</Typography>
                          </Typography>
                          ...
                          <Button fullWidth variant="contained" disabled={loading || !PAYPAL_CLIENT_ID}
                      ...
                      <PayPalButtons
                        ...
                        createOrder={async () => {
                          try {
                            ...
                            return data.id; // PayPal espera el string de order id
                          } catch (err) {
                            console.error('createOrder error:', err);
                            setAlerta(err?.response?.data?.message || err.message || 'Error al iniciar el pago.');
                            throw err;
                          }
                        }}
                        onApprove={async (data) => {
                          try {
                            ...
                            setAlerta(resp?.data?.message || 'Pago capturado y suscripción activada.');
                            setPlanSeleccionado(null);
                            if (empresaId) await cargarDatos(empresaId);
                          } catch (e) {
                            console.error('onApprove error:', e);
                            setAlerta(e?.response?.data?.message || e.message || 'Hubo un problema al procesar el pago.');
                          } finally {
                            setLoading(false);
                          }
                        }}
            ...
            <>
              <Stack direction={{ xs: 'column', md: 'row' }} alignItems={{ xs: 'flex-start', md: 'center' }} justifyContent="space-between" spacing={1.5} sx={{ mb: 1 }}>
                <Typography variant="h6" fontWeight={800}>Plan actual: {planActual?.nombre || '-'}</Typography>
                <Button onClick={() => empresaId && cargarDatos(empresaId)} disabled={loading}
                  startIcon={loading ? <CircularProgress size={16} /> : <AutorenewIcon />} variant="outlined"
                  sx={{ color: '#fff', borderColor: ui.borderSoft, '&:hover': { bgcolor: ui.outlineHover, borderColor: '#fff' } }}>
                  {loading ? 'Actualizando…' : 'Refrescar'}
              ...
              <Paper variant="outlined" sx={{ p: 2, borderRadius: 3, borderColor: ui.borderSoft, bgcolor: '#0b1220' }}>
                <Grid container spacing={2}>
                  <Grid item xs={12} md={4}><Typography variant="body2" sx={{ color: ui.mutted }}>Usuarios permitidos</Typography><Typography variant="h6" fontWeight={800}>{planActual?.limite_usuarios ?? '-'}</Typography></Grid>
                  <Grid item xs={12} md={4}><Typography variant="body2" sx={{ color: ui.mutted }}>Lockers incluidos</Typography><Typography variant="h6" fontWeight={800}>{planActual?.lockers ?? '-'}</Typography></Grid>
                  <Grid item xs={12} md={4}><Typography variant="body2" sx={{ color: ui.mutted }}>Costo mensual</Typography><Typography variant="h6" fontWeight={800}>${toMoney(planActual?.costo)}</Typography></Grid>
                </Grid>
